                        ...
                        @if(request('category'))
                            ...
                            @if($selectedCategory)
                                <span class="badge bg-secondary">
                                    <i class="fas fa-tag"></i> Category: {{ $selectedCategory->category_name }}
                                </span>
                            @endif
                        @endif
                    </div>
            ...
            <div class="card shadow-sm mb-4">
                ...
                <ul class="list-group list-group-flush">
                    ...
                    <li class="list-group-item"
                        style="{{ !request('category') ? 'background-color: #004A9E;' : 'background-color: white;' }}">
                        <a href="{{ route('work.reference', ['search' => request('search')]) }}"
                           class="d-flex justify-content-between align-items-center text-decoration-none"
                           style="color: {{ !request('category') ? 'white' : '#212529' }};">
                            <span>All Categories</span>
                            <span class="badge rounded-pill"
                                  style="background-color: {{ !request('category') ? 'white' : '#004A9E' }};
                                         color: {{ !request('category') ? '#004A9E' : 'white' }};">
                    ...
                    @php
                        $allCategories = \App\Models\Workreferencecategory::withCount('works')->orderBy('category_name')->get();
                    @endphp

                    @foreach($allCategories as $cat)
                    <li class="list-group-item"
                        style="{{ request('category') == $cat->id ? 'background-color: #004A9E;' : 'background-color: white;' }}">
                        <a href="{{ route('work.reference', ['category' => $cat->id, 'search' => request('search')]) }}"
                           class="d-flex justify-content-between align-items-center text-decoration-none"
                           style="color: {{ request('category') == $cat->id ? 'white' : '#212529' }};">
                            <span>{{ $cat->category_name }}</span>
                            <span class="badge rounded-pill"
                                  style="background-color: {{ request('category') == $cat->id ? 'white' : '#004A9E' }};
                                         color: {{ request('category') == $cat->id ? '#004A9E' : 'white' }};">
                                {{ $cat->works_count }}
                            </span>
                        </a>
                    </li>
